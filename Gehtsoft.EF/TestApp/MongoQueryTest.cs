...
namespace TestApp
{
    ...
    public class MongoTestQuery
    {
        ...
        public void TestFilterBuilder()
        {
            ...
            builder.EndGroup();
            ClassicAssert.Throws<EfMongoDbException>(() => builder.ToBsonDocument(), "e2");

            builder.Reset();
            builder.BeginGroup(LogOp.And);
            ClassicAssert.Throws<EfMongoDbException>(() => builder.ToBsonDocument(), "e4");

            builder.Reset();
            builder.BeginGroup(LogOp.And);
            ClassicAssert.Throws<EfMongoDbException>(() => builder.EndGroup(), "e5");

            builder.Reset();
            ClassicAssert.AreEqual("()", builder.ToString(), "a1");
            ...
            ClassicAssert.AreEqual("(f1 $eq 1 $and f2 $ne 2 $and f3 $gt 3 $and f4 $gte 4 $and f5 $lt 5 $and f6 $lte 6)", builder.ToString(), "a2");

            builder.Reset();
            builder.Add(LogOp.Or, "f1", CmpOp.Eq, 1);
            builder.Add(LogOp.Or, "f2", CmpOp.Neq, 2);
            builder.Add(LogOp.Or, "f3", CmpOp.Gt, 3);
            ClassicAssert.AreEqual("(f1 $eq 1 $or f2 $ne 2 $or f3 $gt 3)", builder.ToString(), "a3");

            ClassicAssert.Throws<EfMongoDbException>(() => builder.Add(LogOp.And, "f4", CmpOp.Eq, 5), "e5");

            builder.Reset();
            builder.Add("f1", CmpOp.Eq, 1);
            builder.Add(LogOp.Or, "f2", CmpOp.Neq, 2);
            builder.Add(LogOp.Or, "f3", CmpOp.Gt, 3);
            ClassicAssert.AreEqual("(f1 $eq 1 $or f2 $ne 2 $or f3 $gt 3)", builder.ToString(), "a4");
            ...
            ClassicAssert.AreEqual("(a $eq 1 $or (b $eq 2 $and (c $eq 3 $or d $regex a.*)))", builder.ToString(), "a5");
            ...
            ClassicAssert.AreEqual("(a $in [1, 2, 3] $or b $nin [a, b, c] $or c $eq BsonNull $or d $ne BsonNull)", builder.ToString());
        }
    }
}